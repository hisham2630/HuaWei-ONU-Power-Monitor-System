...
class SSHManager {
  ...
  async connectToControlRouter(controlConfig) {
    return new Promise((resolve, reject) => {
      ...
      conn.connect({
        ...
        // Ignore host key verification (like StrictHostKeyChecking=no)
        hostVerifier: () => true
      });
    });
  }
  ...
  async executeOnLHG60G(controlConfig, deviceConfig, command) {
    return new Promise((resolve, reject) => {
      console.log(`[SSH] Connecting to LHG60G via ${controlConfig.control_ip}:${deviceConfig.mikrotik_ssh_port}`);
      console.log(`[SSH] Username: ${deviceConfig.mikrotik_ssh_username}`);
