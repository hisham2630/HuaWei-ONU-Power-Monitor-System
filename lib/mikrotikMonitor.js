...
class MikroTikMonitor {
  ...
  async monitorMikroTik(deviceConfig) {
    try {
      ...
      // Consider 'no-link' as valid data (not a failure)
      if (rssi === null && (portSpeed === null || portSpeed === undefined)) {
      ...
      console.log(`[MikroTik Monitor] Success - RSSI: ${rssi} dBm, Speed: ${portSpeed}`);
      return {
        success: true,
        data: {
          rssi: rssi,
          rssiUnit: 'dBm',
          portSpeed: portSpeed,
          portSpeedUnit: portSpeed === 'no-link' ? '' : 'Mbps',
  ...
  parsePortSpeed(output) {
    try {
      // Check for no-link status first
      if (output.includes('status: no-link')) {
        return 'no-link';
      }
