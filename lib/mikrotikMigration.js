...
function migrateMikroTikSupport(dbPath = './data/onu_monitor.db') {
  ...
  try {
    ...
    // Always fix monitoring_cache table to ensure proper FOREIGN KEY
    console.log('Fixing monitoring_cache table...');
    try {
      // Check if monitoring_cache has wrong FK reference
      const cacheInfo = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='monitoring_cache'").get();
      if (cacheInfo && cacheInfo.sql.includes('onu_devices_old')) {
        console.log('  Detected broken FOREIGN KEY, recreating monitoring_cache...');
        db.exec('DROP TABLE IF EXISTS monitoring_cache');
        db.exec(`
          CREATE TABLE monitoring_cache (
            device_id INTEGER PRIMARY KEY,
        ...
        console.log('  monitoring_cache table is OK');
      }
    } catch (err) {
      console.log('  Warning: Could not check monitoring_cache:', err.message);
    }
