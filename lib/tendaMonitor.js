...
const TENDA_REBOOT_PAGE = '/admin/reboot.asp';
const TENDA_REBOOT_FORM = '/boaform/admin/formReboot';

function buildBoaSecuredBody(fields) {
    const securityFlag = computePostSecurityFlag(fields);
    return `${buildBoaFormBody(fields)}postSecurityFlag=${securityFlag}`;
}

function isRebootConnectionSuccess(error) {
    const connLost = ['ECONNRESET', 'ECONNABORTED', 'ETIMEDOUT', 'ECONNREFUSED'].includes(error.code);
    const timeoutMsg = error.message && error.message.toLowerCase().includes('timeout');
    return connLost || timeoutMsg;
}

async function submitTendaReboot(client, fields, refererReboot) {
    const rebootBody = buildBoaSecuredBody(fields);
    try {
        const response = await client.post(TENDA_REBOOT_FORM, rebootBody, {
            referer: refererReboot,
            step: 'submit reboot form',
            timeout: 15000
        });
        if (response.status >= 200 && response.status < 300) {
            return { success: true, message: 'Reboot initiated' };
        }
        return { success: false, message: `Reboot failed (HTTP ${response.status})` };
    } catch (error) {
        if (isRebootConnectionSuccess(error)) {
            return { success: true, message: 'Reboot initiated (device connection closed)' };
        }
        return { success: false, message: error.message };
    }
}

async function rebootTendaONUInternal(host, username, password) {
    const normalizedHost = normalizeHost(host);
    const refererReboot = `http://${buildHostHeader(normalizedHost)}${TENDA_REBOOT_PAGE}`;
    const attemptedAt = new Date().toISOString();

    const loginResult = await loginTendaInternal(normalizedHost, username, password);
    if (!loginResult.success) {
        return {
            success: false,
    ...
    const rebootPage = await loginResult.client.get(TENDA_REBOOT_PAGE, {
        referer: refererReboot,
        step: 'load reboot page'
    });

    if (rebootPage.status !== 200) {
        return {
            success: false,
            message: `Failed to load reboot page (HTTP ${rebootPage.status})`,
            attemptedAt
        };
    }

    let result = await submitTendaReboot(loginResult.client, {}, refererReboot);
    if (result.success) {
        return { ...result, attemptedAt };
    }

    result = await submitTendaReboot(loginResult.client, {
        save: 'Commit and Reboot',
        'submit-url': '/reboot.asp'
    }, refererReboot);

    return { ...result, attemptedAt };
}

async function rebootTendaONU(host, username, password) {
    try {
        return await withHostLock(host, () => rebootTendaONUInternal(host, username, password));
    } catch (error) {
        if (isRebootConnectionSuccess(error)) {
            return {
                success: true,
        ...
        logTenda(host, `reboot failed: ${error.message}`);
        return {
            success: false,
