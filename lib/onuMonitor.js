...
// Import Tenda ONU monitor
const { monitorTendaONU, checkTendaConnectivity, rebootTendaONU } = require('./tendaMonitor');

/**
 * ONU Monitoring Module
 * Handles authentication and data extraction from Huawei ONU devices
 * Also supports Tenda ONU devices via the tendaMonitor module
 */
...
const HUAWEI_CFGFILE_PAGE = '/html/ssmp/cfgfile/cfgfile.asp';
const HUAWEI_REBOOT_SET_CGI =
...
/**
 * Extract HW token from Configuration file management page (Save and restart).
 */
function extractOntToken(html) {
  const htmlStr = String(html);
  let match = htmlStr.match(/id=["']hwonttoken["'][^>]*value=["']([^"']+)["']/i);
  if (match) return match[1];
  match = htmlStr.match(/name=["']onttoken["'][^>]*value=["']([^"']+)["']/i);
  if (match) return match[1];
  match = htmlStr.match(/id=["']onttoken["'][^>]*value=["']([^"']+)["']/i);
  return match ? match[1] : '';
}

function buildCookieHeader(sessionCookies) {
...
/**
 * Reboot Huawei ONU (Blue or Red UI) via System Tools → Configuration File → Save and restart.
 */
async function rebootHuaweiONU(host, username, password) {
  const attemptedAt = new Date().toISOString();

  try {
    const loginResult = await login(host, username, password);
    if (!loginResult.cookies?.Cookie || !loginResult.cookies.Cookie.includes('sid')) {
      return {
        success: false,
    ...
    const cfgResponse = await loginResult.apiClient.get(HUAWEI_CFGFILE_PAGE, {
      headers: commonHeaders
    });

    if (cfgResponse.status !== 200 || String(cfgResponse.data).length < 500) {
      return {
        success: false,
        message: 'Failed to load configuration file management page',
        attemptedAt
      };
    }

    const hwToken = extractOntToken(cfgResponse.data);
    if (!hwToken) {
      return {
        success: false,
        message: 'Failed to obtain reboot token from ONU',
        attemptedAt
      };
    }

    const rebootData = new URLSearchParams();
    rebootData.append('x.X_HW_Token', hwToken);

    try {
      await loginResult.apiClient.post(HUAWEI_REBOOT_SET_CGI, rebootData.toString(), {
        headers: {
          ...commonHeaders,
          'Content-Type': 'application/x-www-form-urlencoded',
          Referer: `http://${host}${HUAWEI_CFGFILE_PAGE}`
        },
        timeout: 15000
      });
      return {
      ...
      const connLost = ['ECONNRESET', 'ECONNABORTED', 'ETIMEDOUT', 'ECONNREFUSED'].includes(
        postError.code
      );
      const timeoutMsg = postError.message && postError.message.toLowerCase().includes('timeout');
      if (connLost || timeoutMsg) {
        return {
          success: true,
          message: 'Reboot initiated (device connection closed)',
          attemptedAt
        };
      }
      throw postError;
...
async function rebootBlueONU(host, username, password) {
  return rebootHuaweiONU(host, username, password);
}

async function rebootRedONU(host, username, password) {
  return rebootHuaweiONU(host, username, password);
}

/**
 * Reboot by ONU type (blue/red Huawei only).
 */
async function rebootONU(host, username, password, onuType) {
  if (onuType === 'blue' || onuType === 'red') {
    return rebootHuaweiONU(host, username, password);
  }
  if (onuType === 'tenda') {
    return rebootTendaONU(host, username, password);
  }
  return {
    success: false,
    message: `Reboot not supported for ONU type: ${onuType}`,
