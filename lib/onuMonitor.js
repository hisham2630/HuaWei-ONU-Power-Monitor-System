...
async function getEthernetPortSpeeds(apiClient, sessionCookies, host) {
  ...
  // Extract the geInfos array with a more flexible regex
  const geInfosMatch = html.match(/var\s+geInfos\s*=\s*new\s+Array\s*\(\s*(.*?)\s*\)\s*;/s);
  if (geInfosMatch && geInfosMatch[1]) {
    // Extract GEInfo objects from the array
    // Handle both formats: with and without quotes around parameters
    const geInfoObjects = geInfosMatch[1].match(/new\s+GEInfo\s*\(\s*[^)]+\s*\)/g);
    
    if (geInfoObjects && geInfoObjects.length > 0) {
      // Process each GEInfo object
      for (let i = 0; i < geInfoObjects.length; i++) {
        // Extract all parameters from the GEInfo constructor
        const paramsMatch = geInfoObjects[i].match(/new\s+GEInfo\s*\(\s*(.*?)\s*\)/);
        if (paramsMatch && paramsMatch[1]) {
          ...
          // Speed is typically the third parameter (index 2)
          if (params.length >= 3) {
            const speedCode = params[2];
            // Convert speed code to Mbps value
            // "2" = 1000 Mbps, "1" = 100 Mbps, "0" = 10 Mbps or unknown
            let speedValue;
            switch (speedCode) {
              case '2':
                speedValue = 1000;
            ...
            // Port numbering typically starts at 1
