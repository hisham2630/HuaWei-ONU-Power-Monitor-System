...
class NotificationService {
  ...
  shouldNotify(device, monitoringData) {
    // Route to appropriate checker based on device type
    if (device.device_type === 'mikrotik_lhg60g') {
      return this.shouldNotifyMikroTik(device, monitoringData);
    } else {
      return this.shouldNotifyONU(device, monitoringData);
    }
  }

  /**
   * Check if notification should be sent for MikroTik devices
   */
  shouldNotifyMikroTik(device, monitoringData) {
    const alerts = [];
    const formattedDeviceName = this.getFormattedDeviceName(device);

    console.log(`Checking MikroTik notifications for device ${formattedDeviceName}:`);
    console.log(`  Monitoring data:`, monitoringData);

    // Check RSSI
    if (device.notifyRssi && monitoringData.rssi !== null && monitoringData.rssi !== undefined) {
      console.log(`  RSSI Check: enabled=${device.notifyRssi}, value=${monitoringData.rssi}, threshold=${device.rssiThreshold}`);
      
      if (monitoringData.rssi < device.rssiThreshold) {
        alerts.push({
          type: 'rssi_low',
          message: `Low RSSI Alert: ${formattedDeviceName} - Current: ${monitoringData.rssi} dBm, Threshold: ${device.rssiThreshold} dBm`
        });
        console.log(`  ✓ Low RSSI alert triggered!`);
      } else {
        console.log(`  ✗ RSSI alert NOT triggered (${monitoringData.rssi} >= ${device.rssiThreshold})`);
      }
    } else {
      console.log(`  RSSI notifications disabled or no data`);
    }

    // Check Port Speed
    if (device.notifyPortSpeed && monitoringData.portSpeed !== null && monitoringData.portSpeed !== undefined) {
      console.log(`  Port Speed Check: enabled=${device.notifyPortSpeed}, value=${monitoringData.portSpeed}, threshold=${device.portSpeedThreshold}`);
      
      if (monitoringData.portSpeed < device.portSpeedThreshold) {
        alerts.push({
          type: 'port_speed_low',
          message: `Port Speed Alert: ${formattedDeviceName} - Current: ${monitoringData.portSpeed} Mbps, Threshold: ${device.portSpeedThreshold} Mbps`
        });
        console.log(`  ✓ Port speed alert triggered!`);
      } else {
        console.log(`  ✗ Port speed alert NOT triggered (${monitoringData.portSpeed} >= ${device.portSpeedThreshold})`);
      }
    } else {
      console.log(`  Port speed notifications disabled or no data`);
    }
  ...
  /**
   * Check if notification should be sent for ONU devices
   */
  shouldNotifyONU(device, monitoringData) {
    const alerts = [];
    const formattedDeviceName = this.getFormattedDeviceName(device);

    console.log(`Checking ONU notifications for device ${formattedDeviceName}:`);
    console.log(`  Monitoring data:`, monitoringData);
  ...
  async processMonitoringResult(device, result) {
    const formattedDeviceName = this.getFormattedDeviceName(device);
    const deviceHost = device.device_type === 'mikrotik_lhg60g' ? device.mikrotik_lhg60g_ip : device.host;
    ...
    if (!result.success) {
      ...
      if (device.notifyOffline) {
        ...
        if (newFailures >= device.retryAttempts && !device.isOfflineNotified) {
          ...
          const message = `Device Offline: ${formattedDeviceName} at ${deviceHost} is not responding after ${newFailures} attempts.`;
    ...
    if (device.notifyOffline && device.isOfflineNotified) {
      ...
      const message = `Device Online: ${formattedDeviceName} at ${deviceHost} is back online.`;
      await this.sendNotificationToAll(message);
