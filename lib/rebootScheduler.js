const { rebootONU, checkConnectivity } = require('./onuMonitor');

/**
 * Reboot Scheduler
 * Runs scheduled Huawei ONU reboots (Blue/Red UI) on weekday + local time.
 */
...
class RebootScheduler {
  ...
  alreadyRanThisSlot(schedule, now) {
    ...
    return (
      ...
      this.formatLocalTime(last) === schedule.timeLocal
    );
  }
  ...
  async tick() {
    ...
    for (const schedule of this.schedules) {
      if (schedule.timeLocal !== currentTime) continue;
      if (!this.isDayEnabled(schedule.daysMask, dayIndex)) continue;
      if (this.alreadyRanThisSlot(schedule, now)) continue;
      if (this.inProgress.has(schedule.deviceId)) continue;

      await this.runScheduledReboot(schedule, now);
    }
  }

  async runScheduledReboot(schedule, now) {
    const deviceId = schedule.deviceId;
    const scheduledFor = `${now.toISOString().slice(0, 10)} ${schedule.timeLocal}`;
    this.inProgress.add(deviceId);

    try {
      const reachable = await checkConnectivity(schedule.deviceHost);
      if (!reachable) {
