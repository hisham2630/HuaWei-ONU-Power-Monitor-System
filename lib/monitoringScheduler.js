const { monitorONU, getEthernetPortSpeeds, monitorTendaONU } = require('./onuMonitor');
  ...
  async monitorWithRetry(device) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (device.device_type === 'mikrotik_lhg60g') {
        ...
        } else if (device.onuType === 'tenda') {
          // Tenda ONU device monitoring - doesn't support port speeds
          ...
          const includePortSpeeds = device.showPortSpeeds === true && device.portSelections && device.portSelections.length > 0;
          const includeOnlineDuration = device.showOnlineDuration === true;
