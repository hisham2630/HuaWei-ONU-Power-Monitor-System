...
async function getEthernetPortSpeeds() {
  try {
    ...
    if (isRedUI && !hasGeInfos) {
      console.log('Using Red UI HTML table parsing');
        ...
        if (geInfosMatch && geInfosMatch[0]) {
          ...
          if (geInfoObjects) {
            ...
            for (let i = 0; i < geInfoObjects.length; i++) {
              ...
              // 2nd: Status/Link (0 = disconnected, 1 = connected)
              ...
              if (paramsMatch) {
                const portStatus = paramsMatch[1]; // Second parameter (Status/Link)
                const speedCode = paramsMatch[2];  // Third parameter (Speed code)
                const portNumber = i + 1;
                
                // If port status is "0", it means the port is disabled/disconnected
                if (portStatus === '0') {
                  portSpeeds[`eth${portNumber}-speed`] = 0; // 0 indicates disconnected port
                } else {
    ...
    // Display port speeds with proper handling for disconnected ports (speed = 0)
    for (const [portKey, speed] of Object.entries(portSpeeds)) {
      const portNumber = portKey.replace('eth', '').replace('-speed', '');  
      if (speed === 0) {
        console.log(`Port ${portNumber} Speed: -- (Disconnected)`);
      } else {
        console.log(`Port ${portNumber} Speed: ${speed} Mbps`);
      }
    }
    
    if (Object.keys(portSpeeds).length === 0) {
      console.log('No port speed information found');
    }
    console.log('=============================\n');
